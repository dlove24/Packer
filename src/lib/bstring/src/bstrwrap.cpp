...
const CBString CBString::operator + (const char* s) const {
  if (s == NULL) {
    bstringThrow ("Failure in + (char *) operator, NULL");
    }
...
const CBString CBString::operator + (const unsigned char* s) const {
  if (s == NULL) {
    bstringThrow ("Failure in + (unsigned char *) operator, NULL");
    }
...
void CBString::join (const struct CBStringList& l) {
  ...
  for (c = 1, i = 0; i < l.size(); i++) {
    ...
    if (c < 0) {
      bstringThrow ("Failure in (CBStringList) constructor, too long");
      }
    }
...
void CBStringList::split (const CBString& b, unsigned char splitChar) {
  ...
  do {
    for (i = p; i < b.length (); i++) {
      if (b.character (i) == splitChar) {
        break;
        }
      }

    if (i >= p) {
      this->push_back (CBString (& (b.data[p]), i - p));
      }
...
void CBStringList::split (const CBString& b, const CBString& s) {
  ...
  if (s.length() == 0) {
    bstringThrow ("Null splitstring failure");
    }
  ...
  if (s.length() == 1) {
    ...
    do {
      for (i = p; i < b.length (); i++) {
        ...
        if (chrs.content[c >> 5] & ( (long) 1) << (c & 31)) {
          break;
          }
        }
...
std::istream& getline (std::istream& sin, CBString& b, char terminator) {
  ...
  if (b.slen > 0 && b.data[b.slen - 1] == terminator) {
    b.slen--;
    }
